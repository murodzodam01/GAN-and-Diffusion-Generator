...
The complete analysis, saved outputs, visualizations, and model implementations are available in `MAGIC_Generative_Models.ipynb`.
Motivation
...
Dataset
The project uses the MAGIC Gamma Telescope dataset from the UCI Machine Learning Repository.
Observations: 19,020
...
Project workflow
Load and inspect the MAGIC event data.
Create a stratified train–test split.
Normalize long-tailed features with `QuantileTransformer`.
Train a class-conditional WGAN.
Train a compact diffusion-based neural network.
Return generated samples to the original feature space.
Compare real and synthetic marginal distributions.
Evaluate realism with independent logistic-regression and gradient-boosting classifiers.
Quantify real-versus-generated separability using accuracy and ROC AUC.
Models
Conditional WGAN
...
label real observations as `1` and generated observations as `0`;
train an independent classifier to separate the two groups;
measure its performance on held-out data.
